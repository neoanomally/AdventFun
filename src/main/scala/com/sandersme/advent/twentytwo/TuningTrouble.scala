...
import com.sandersme.advent.twentytwo.model.CommDecoder
...
object TuningTrouble {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val input = Input.readTwentyTwoFromResource("day6_input")

    val startOfPacket = CommDecoder.findStartOfPacket(input.head)

    println(s"Start of packet is at position ${startOfPacket}")

    val startOfMessage = CommDecoder.findStartOfMessage(input.head)

    println(s"Start of message is at position ${startOfMessage}")
  }
}
