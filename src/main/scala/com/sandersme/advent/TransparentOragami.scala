...
import com.sandersme.advent.model.FoldableGrid
import com.sandersme.advent.model.FoldableGrid.{foldAllInstructions, foldOneStep}
...
dots that end up overlapping after the fold is completed count as a single dot.
  ...
  def main(args: Array[String]): Unit = {
    ...
    val oneStepFold = foldOneStep(foldableGrid)
    println(s"The number of visible dots: ${oneStepFold.countVisibleDots}")

    val foldedAllStpes = FoldableGrid.foldAllInstructions(foldableGrid)
    println("ALL STEPS FOLDED")
    foldedAllStpes.printGrid()
  }
}
